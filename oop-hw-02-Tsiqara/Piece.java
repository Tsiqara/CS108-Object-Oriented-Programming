...
public class Piece {
	...
	*/
	...
	private int[] calcSkirt(){
		...
		Arrays.fill(result, height);
		for(TPoint p : body){
			result[p.x] = Math.min(p.y, result[p.x]);
		}
		return result;
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] p = new TPoint[body.length];
		for(int i = 0; i < body.length; i ++) {
			p[i] = new TPoint(height - 1 - body[i].y, body[i].x);
		}
		return new Piece(p);
	}
	 ...
	 * public Piece computeNextRotation() {
		 ...
		 for(int i = 0; i < body.length; i ++){
		 p[i] = new TPoint(body[i].y, body[i].x);
		 }
		 return new Piece(p);
		 }

		 //others swap and mirror by x=0,5 or x=1
		 for(int i = 0; i < body.length; i ++){
		 int y = body[i].x;
		 int x;
		 if(width % 2 == 0){
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		while(!root.equals(next)){
			cur = next;
			next.next = next.computeNextRotation();
			next = next.next;
		}
		cur.next = root;
		return root;
